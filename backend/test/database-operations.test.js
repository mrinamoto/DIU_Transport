...
test('fresh initialization applies every migration at the current schema version',()=>{
  const db=new Database(databasePath,{readonly:true});
  try{assert.equal(db.prepare('SELECT MAX(version) AS version FROM schema_migrations').get().version,MIGRATION_VERSION);assert.ok(db.prepare("SELECT 1 FROM sqlite_schema WHERE type='table' AND name='audit_logs'").get());assert.ok(db.prepare("SELECT 1 FROM sqlite_schema WHERE type='table' AND name='route_stops'").get());assert.ok(db.prepare("SELECT 1 FROM sqlite_schema WHERE type='table' AND name='special_trips'").get());}
  finally{db.close();}
});
...
test('a Phase 3 database upgrades non-destructively to Phase 5',()=>{
  ...
  legacy.exec(fs.readFileSync(path.join(__dirname,'../src/db/migrations/001_phase2_baseline.sql'),'utf8'));
  legacy.prepare("INSERT INTO schema_migrations(version,name) VALUES(1,'phase2_baseline')").run();
  legacy.exec(fs.readFileSync(path.join(__dirname,'../src/db/migrations/002_phase3_catalog_audit.sql'),'utf8'));
...
test('SQLite-safe backup and temporary restore verification preserve the active database',async()=>{
  ...
  const summary=verifyBackup(backupPath,{sourcePath:databasePath});assert.equal(summary.schemaVersion,MIGRATION_VERSION);assert.ok(summary.tables.includes('audit_logs'));assert.ok(summary.tables.includes('notifications'));assert.equal(hashFile(databasePath),beforeHash);assert.ok(fs.existsSync(backupPath));
});
...
test('sanitized export manifest preserves structure without credential fields',()=>{
  ...
  assert.equal(payload.manifest.schema_version,MIGRATION_VERSION);assert.ok(payload.manifest.tables.some((item)=>item.table==='users'));assert.ok(payload.data.users.every((row)=>!('password_hash'in row)&&!('email'in row)&&!('full_name'in row)));assert.ok(payload.data.drivers.every((row)=>!('phone'in row)&&!('full_name'in row)));assert.ok(payload.data.employees.every((row)=>!('phone'in row)&&!('email'in row)&&!('full_name'in row)&&!('notes'in row)));assert.ok(payload.data.special_trips.every((row)=>!('title'in row)&&!('description'in row)&&!('organizer'in row)));assert.ok(payload.data.notifications.every((row)=>!('title'in row)&&!('message'in row)));assert.ok(payload.data.emergency_contacts.every((row)=>!('phone'in row)&&!('email'in row)&&!('name'in row)));assert.ok(payload.data.feedback.every((row)=>!('subject'in row)&&!('message'in row)&&!('admin_response'in row)));assert.equal(hashFile(databasePath),beforeHash);
});
